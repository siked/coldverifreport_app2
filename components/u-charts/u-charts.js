...
function getToolTipData(seriesData, calPoints, index, categories) {
  ...
  var textList = seriesData.map(function(item) {
    return {
      ...
      text: option.format ? option.format(item, categories[index], index) : item.name + ': ' + item.data,
      color: item.color
    };
  });
  // 支持 tooltip 顶部标题（比如时间），单独一行展示
  // - option.title: string | function(category, index, categories) => string
  if (option && option.title) {
    var titleText = typeof option.title === 'function' ? option.title(categories[index], index, categories) : option.title;
    if (titleText !== undefined && titleText !== null && String(titleText).length > 0) {
...
function calCategoriesData(categories, opts, config, eachSpacing) {
  ...
  // 支持两行X轴标签：使用 "|" 作为换行符（例如 "MM-DD|HH:MM"）
  ...
  // 非旋转时，若存在两行标签，增加X轴高度以容纳两行文字
  if (hasMultiLine && result.angle === 0) {
    var xAxisFontSize = opts.xAxis.fontSize || config.fontSize;
    var multiLineHeight = 2 * config.xAxisTextPadding + xAxisFontSize * 2 + 2;
  ...
  if (opts.xAxis.disabled !== true) {
    ...
    if (config._xAxisTextAngle_ === 0) {
      newCategories.forEach(function(item, index) {
        ...
        // 第一行：日期
        context.fillText(line1, xAxisPoints[index] + offset, startY + xAxisFontSize + (config.xAxisHeight - xAxisFontSize) / 2);
        // 第二行：时间（若存在）
        if (line2) {
          context.fillText(line2, xAxisPoints[index] + offset, startY + xAxisFontSize * 2 + (config.xAxisHeight - xAxisFontSize) / 2);
        }
        context.closePath();
        context.stroke();
      });
...
// 兼容 H5/ESM：提供默认导出，避免 `does not provide an export named 'default'`
